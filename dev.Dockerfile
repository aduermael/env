...
ARG GROK_CLI_VERSION=1.0.36
ARG GROK_CLI_SHA256_AMD64=90e373f48b0fd5b6fb2af4e92a6e59ff4fb03e97628715db74bb57dcf89c2f01
ARG GROK_CLI_SHA256_ARM64=3abe9b0eabdab906bb3c491b760621009a56664a2f40d24f51efa4eb8b29768e
RUN set -eux; \
    image_arch="${TARGETARCH:-$(dpkg --print-architecture)}"; \
    case "${image_arch}" in \
...
ARG CODEX_VERSION=rust-v0.155.0
...
ARG CODEX_CODE_MODE_HOST_SHA256_AMD64=328c1bebe09fc727053794576efea353d3b18381ee8883f5283b37ae4a7854d4
ARG CODEX_CODE_MODE_HOST_SHA256_ARM64=0ecd8e263468b520770c6dc6c9ebafeba3052c0c9796eabc5b0c469f77d5489b
RUN set -eux; \
    image_arch="${TARGETARCH:-$(dpkg --print-architecture)}"; \
    case "${image_arch}" in \
        amd64|x86_64) \
...
ARG CURSOR_CLI_VERSION=2026.09.15-d2fe57e
ARG CURSOR_CLI_SHA256_AMD64=4b7b026dd104e935b216cc52f905a560d741fc80a4a4d62ef655735b96a15c97
ARG CURSOR_CLI_SHA256_ARM64=2d741c12c3ee7a505584579efb28a0ee31ff13fefc1f347e2d3b43688c04620d
RUN set -eux; \
    image_arch="${TARGETARCH:-$(dpkg --print-architecture)}"; \
    case "${image_arch}" in \
