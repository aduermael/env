...
usage() {
    cat <<'EOF'
Usage: update-grok-pr.sh [latest|VERSION]

Runs ./scripts/update-grok.sh from a fresh origin/main worktree.
...
main() {
    ...
    [[ -x "${repo_root}/scripts/update-grok.sh" ]] || die "missing executable: ./scripts/update-grok.sh"
    ...
    tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/update-grok-pr.XXXXXX")"
    ...
    if [[ "$#" -eq 1 ]]; then
        (cd "$worktree" && ./scripts/update-grok.sh "$1")
    else
        (cd "$worktree" && ./scripts/update-grok.sh)
    fi
    ...
    version="$(sed -n 's/^ARG GROK_CLI_VERSION=//p' "${worktree}/dev.Dockerfile" | head -n 1)"
    [[ -n "$version" ]] || die "could not read GROK_CLI_VERSION from dev.Dockerfile"

    base_branch="update-grok-$(sanitize_branch_component "$version")"
    branch="$base_branch"
    ...
    git -C "$worktree" commit -m "Update Grok Build to ${version}"
    git -C "$worktree" push -u origin "$branch"

    cat > "$body_file" <<EOF
## Summary
- update Grok Build to ${version} in the dev image
- update amd64 and arm64 checksums for the new release

## Tests
- ./scripts/update-grok.sh${1:+ $1}
EOF

    pr_url="$(git -C "$worktree" rev-parse --show-toplevel >/dev/null && cd "$worktree" && gh pr create --base main --head "$branch" --title "Update Grok Build to ${version}" --body-file "$body_file")"
