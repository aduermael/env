...
usage() {
    cat <<'EOF'
Usage: update-cursor-pr.sh [latest|VERSION]

Runs ./scripts/update-cursor.sh from a fresh origin/main worktree.
...
main() {
    ...
    [[ -x "${repo_root}/scripts/update-cursor.sh" ]] || die "missing executable: ./scripts/update-cursor.sh"
    ...
    tmpdir="$(mktemp -d "${TMPDIR:-/tmp}/update-cursor-pr.XXXXXX")"
    ...
    if [[ "$#" -eq 1 ]]; then
        (cd "$worktree" && ./scripts/update-cursor.sh "$1")
    else
        (cd "$worktree" && ./scripts/update-cursor.sh)
    fi
    ...
    version="$(sed -n 's/^ARG CURSOR_CLI_VERSION=//p' "${worktree}/dev.Dockerfile" | head -n 1)"
    [[ -n "$version" ]] || die "could not read CURSOR_CLI_VERSION from dev.Dockerfile"

    base_branch="update-cursor-$(sanitize_branch_component "$version")"
    branch="$base_branch"
    ...
    git -C "$worktree" commit -m "Update Cursor CLI to ${version}"
    git -C "$worktree" push -u origin "$branch"

    cat > "$body_file" <<EOF
## Summary
- update Cursor CLI to ${version} in the dev image
- update amd64 and arm64 checksums for the new release

## Tests
- ./scripts/update-cursor.sh${1:+ $1}
EOF

    pr_url="$(git -C "$worktree" rev-parse --show-toplevel >/dev/null && cd "$worktree" && gh pr create --base main --head "$branch" --title "Update Cursor CLI to ${version}" --body-file "$body_file")"
